...
const Editor = ({}) => {
  ...
  const saveToServer = async (file) => {
    ...
    const res = await fetch(
      "https://api.cloudinary.com/v1_1/dlgwvuu5d/image/upload",
      { method: "POST", body }
    ).then((r) => r.json());
  ...
  const submitHandler = () => {
    ...
    axios
      .post("http://localhost:5000/bloggerDashboard/add", data)
      .then((res) => {
  ...
  React.useEffect(() => {
    if (quill) {
      quill.clipboard.dangerouslyPasteHTML(
        "<h1>you dont need to enter your Post title here</h1>"
      );
  ...
  return (
    ...
    >
      ...
      <div
        style={{ width: "90%", margin: "auto", height: 577, maxHeight: 577 }}
      >
        <div ref={quillRef} />
      </div>
